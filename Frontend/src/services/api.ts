...
export const configApi = {
  ...
  getRules: () => api.get('/config/discount-tiers'),
  updateRules: (data: any) => api.post('/config/discount-tiers', data),
};

export const rulesApi = configApi;
...
export const warehouseApi = {
  ...
  getInventory: (warehouseId?: string) => api.get('/warehouses/stocks', { params: { warehouse_id: warehouseId } }),
  getSplits: (orderId: string) => api.get(`/warehouses/splits/${orderId}`),
};

export const warehousesApi = warehouseApi;
...
export const approvalsApi = {
  ...
  approve: (id: string, notes?: string) => api.post(`/approvals/${id}/act`, { action: 'approve', note: notes }),
  reject: (id: string, notes?: string) => api.post(`/approvals/${id}/act`, { action: 'reject', reason: notes }),
...
export const billingApi = {
  ...
  get: (id: string) => api.get(`/billing/invoices/${id}`),
  payInvoice: (invId: string, data: any) => api.post(`/billing/invoices/${invId}/pay`, data),
  recordPayment: (id: string, amount: number, method: string) => api.post(`/billing/invoices/${id}/pay`, { amount, method }),
...
export const portalApi = {
  ...
  getQuote: (token: string) => api.get(`/portal/${token}`),
  ...
  counterOffer: (token: string, data: any) => api.post(`/portal/${token}/counter`, data),
  accept: (token: string) => api.post(`/portal/${token}/accept`),
};
...
export const dealHealthApi = {
  ...
  dismissAlert: (id: string) => api.post(`/deal-health/alerts/${id}/dismiss`),
};

export const reportsApi = {
  getSummary: (params: any) => api.get('/reports', { params }),
  getMarginTrends: () => api.get('/reports/margins'),
